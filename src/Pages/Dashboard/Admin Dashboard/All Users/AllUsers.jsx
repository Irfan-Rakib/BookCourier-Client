...
const AllUsers = () => {
  ...
  const removeRole = async (userId) => {
    if (!window.confirm("Remove user's role?")) return;
    try {
      await axios.patch(
        `https://book-courier-server-snowy.vercel.app/admin/users/${userId}`,
        { role: "user" }
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    {user.role === "user" && (
                      <>
                        <button
                          onClick={() =>
                            updateRole(user._id || user.uid, "librarian")
                          }
                          className="btn btn-sm btn-secondary"
                        >
                          Make Librarian
                        ...
                        <button
                          onClick={() =>
                            updateRole(user._id || user.uid, "admin")
                          }
                          className="btn btn-sm btn-secondary"
                        >
                          Make Admin
                    ...
                    {user.role === "librarian" && (
                      <>
                        ...
                        <button
                          onClick={() => removeRole(user._id || user.uid)}
                          className="btn btn-sm btn-error"
                        >
                          Remove Librarian
                        </button>
                    ...
                    {/* Buttons for 'admin' */}
                    {user.role === "admin" &&
                      !["irfanrakib756@gmail.com", "admin@gmail.com"].includes(
                        user.email
                      ) && (
                        <button
                          onClick={() => removeRole(user._id || user.uid)}
